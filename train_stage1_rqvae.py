    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for i, (_, secret, _) in enumerate(tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} [Val]", leave=False)):
                ...
                if i == 0:
                    vis = torch.cat([secret, recon], dim=0)
                    save_image_grid(vis, os.path.join(res_dir, f"val_epoch_{epoch}.png"), nrow=secret.shape[0])
        
        val_psnr_avg /= len(val_loader)
        val_ssim_avg /= len(val_loader)
        
        logger.info(f"Epoch {epoch} Done. Train Loss: {train_loss_avg:.4f} | Val PSNR: {val_psnr_avg:.2f} | Val SSIM: {val_ssim_avg:.4f}")
